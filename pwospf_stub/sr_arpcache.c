...
void sr_arpcache_sweepreqs(struct sr_instance *sr) { 
	...
	struct sr_arpreq *requests = arpcache->requests;
	struct sr_arpreq *current, *aux = requests;
	while(aux) {
		current = aux;
		aux = aux->next;
		time_t now = time(NULL);
		if (difftime(now, current->sent) >= 1) {
			if(current->times_sent >= 5) {
				...
				while (packets) {
					...
					icmp_error(sr, ip_hdr, 3, 1);
					packets = packets->next;
				}
				sr_arpreq_destroy(arpcache, current);
			} else {
