...
struct sr_if* get_if_by_ip(struct sr_if* if_list, uint32_t tip) {
	assert(tip);
	while(if_list){
		if((if_list->ip & if_list->mask) == (tip & if_list->mask)) {
			return if_list;
		}
...
struct sr_if* get_if_by_ip_no_mask(struct sr_if* if_list, uint32_t tip) {
	assert(tip);
	while(if_list){
		if(if_list->ip == tip) {
			return if_list;
